...
func (meta *Meta) configure() {
	...
	if meta.FieldStruct != nil {
		if injector, ok := reflect.New(meta.FieldStruct.FieldType).Interface().(resource.ConfigureMetaBeforeInitializeInterface); ok {
	...
	if hasColumn {
		fieldType = meta.FieldStruct.FieldType
	...
	if hasColumn {
		...
		if meta.Type == "" {
			...
			if rel != nil {
				if rel.Type == gorm.HasOne {
					meta.Type = "single_edit"
				} else if rel.Type == gorm.HasMany {
					meta.Type = "collection_edit"
				} else if rel.Type == gorm.BelongsTo {
					meta.Type = "select_one"
				} else if rel.Type == gorm.Many2Many {
			...
			if rel != nil {
				if (rel.Type == gorm.HasOne || rel.Type == gorm.HasMany) && meta.Meta.Setter == nil && (meta.Type == "select_one" || meta.Type == "select_many") {
					meta.SetSetter(func(resource interface{}, metaValue *resource.MetaValue, context *qor.Context) {
						...
						schema, _ := gorm.Parse(resource)
						if _, zero := schema.PrioritizedPrimaryField.ValueOf(reflect.ValueOf(resource)); !zero {
